...
int main (int argc, char **argv)
{
	...
	rxcfg.fs_hz = MHZ(5.0);   // 5 MS/s rx sample rate
	...
	txcfg.fs_hz = MHZ(5.0);   // 5 MS/s tx sample rate
	...
	while (!stop)
	{
		...
		float freq = 2.0 * M_PI * 10; // 10hz sine wave (should be easy to spot);
		double ampl = 32767;

		double i = 1. / txcfg.fs_hz;

		for (p_dat = iio_buffer_first(txbuf, tx0_i); p_dat < p_end; p_dat += p_inc) {
			// Example: fill with sine wave
